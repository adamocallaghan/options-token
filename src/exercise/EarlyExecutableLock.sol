...
// /// The discount in this contract slowly accrues overtime until it reaches the maxMultiplier.
// /// The oTokens may be exercised at anytime and will receive the discount that has accrued up to that point.
...
//     event MaxMultiplierChanged(uint40 newLockedTime);
...
//     /// @notice The time when the maxDiscount if fully realized
//     uint40 public maxDiscountTime;
...
//         IOracle oracle_,
//         uint256 maxMultiplier_,
//         uint40 maxDiscountTime_,
...
//         /// maxDiscountTime must be in the future
//         if (block.timestamp > maxDiscountTime_) revert Exercise__Multiplier();
//         paymentToken = paymentToken_;
//         underlyingToken = underlyingToken_;
//         lockedUntilTime = lockedUntilTime_;
//         maxDiscountTime = maxDiscountTime_;

//         _setOracle(oracle_);
//         _setMultiplier(maxMultiplier_);
...
//     {
//         return _exercise(from, amount, recipient);
//     }
...
//     /// @notice Sets the discount - maxMultiplier.
//     /// @param multiplier_ The new maxMultiplier
//     function changeMultiplier(uint256 multiplier_) external onlyOwner {
...
//         //@audit using the param amount here does not take into consideration the fees that have been taken. 
//         //distributeFeesFrom should return the remaining amount of tokens and we should pass that to the _pay function
