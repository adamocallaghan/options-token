...
contract SablierStreamCreator {
    ...
    function createLinearStream(uint40 cliffDuration_, uint40 totalDuration_, uint128 amount_, address token_, address recipient_) public returns (uint256 streamId) {
    ...
    function createDynamicStream(uint128 totalAmount_, uint256 amount0_, uint256 amount1_, address token_, address recipient_ ) public returns (uint256 streamId)  {
        // Transfers the tokens to be streamed to this contract @note maybe this needs to go somewhere else 
        IERC20(token_).transferFrom(msg.sender, address(this), amount_);
