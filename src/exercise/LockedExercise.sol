...
abstract contract LockedExercise is BaseExercise {
    /// Library usage
    ...
    event LockedUntilTimeChanged(uint40 newLockedTime);
    ...
    function changeLockedTime(uint40 newLockedTime_) external onlyOwner {
        if (block.timestamp > newLockedTime_) revert Exercise__InvalidLockTime();
        lockedUntilTime = newLockedTime_;
        emit LockedUntilTimeChanged(newLockedTime_);
    }
